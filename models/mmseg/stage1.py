...
class MatVisionTransformer(timm.models.vision_transformer.VisionTransformer):
    def __init__(self, embed_dim, mlp_ratio, depth, num_heads, qkv_bias, out_channels, **kwargs):
        super(MatVisionTransformer, self).__init__(embed_dim=embed_dim, mlp_ratio=mlp_ratio, depth=depth, num_heads=num_heads, dynamic_img_size=True, **kwargs)
        self.depth = depth
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = self.conv_subdim(x, subdim=self.sub_dim)
...
if __name__ == '__main__':
    model = MatVisionTransformer(
        ...
        norm_layer=partial(nn.LayerNorm, eps=1e-6), out_channels=768)
