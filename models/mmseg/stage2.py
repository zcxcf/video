...
class ModifiedAttention(nn.Module):
    ...
    def __init__(
        ...
        self.router = nn.Sequential(nn.Linear(1, 16),
                                    nn.ReLU(),
                                    nn.Linear(16, 16),
                                    nn.ReLU(),
                                    nn.Linear(16, 4),
...
class MatVisionTransformer_stage2(timm.models.vision_transformer.VisionTransformer):
    def __init__(self, embed_dim, mlp_ratio, depth, num_heads, qkv_bias, out_channels, block, **kwargs):
        super(MatVisionTransformer_stage2, self).__init__(embed_dim=embed_dim, mlp_ratio=mlp_ratio, depth=depth, num_heads=num_heads, block_fn=block, dynamic_img_size=True, **kwargs)
        self.depth = depth
        ...
        self.embed_router = nn.Sequential(nn.Linear(1, 16),
                                    nn.ReLU(),
                                    nn.Linear(16, 16),
                                    nn.ReLU(),
                                    nn.Linear(16, 4),
        ...
        self.down32 = nn.Conv2d(out_channels, out_channels, 3, 2, 1)


    def conv_subdim(self, x: torch.Tensor,
                    subdim: int) -> torch.Tensor:

        conv = self.proj

        w = conv.weight[:, :subdim, :, :]
        b = conv.bias
        return F.conv2d(
            x, w, b,
            stride=conv.stride,
    ...
    def forward_features(self, x: torch.Tensor):
        ...
        # ——reshape token → feature map 1/16——
        Hp = H // 16
        Wp = W // 16
        x = x.transpose(1, 2).reshape(B, -1, Hp, Wp)  # (B, embed_dim, 32, 32)

        d_embed = int(torch.sum(self.embed_mask) * self.head_dim)

        x = self.conv_subdim(x, subdim=d_embed)

        feat_1_16 = x
        feat_1_8 = F.interpolate(x, scale_factor=2, mode='bilinear', align_corners=False)  # 64×64
        feat_1_4 = F.interpolate(x, scale_factor=4, mode='bilinear', align_corners=False)  # 128×128
        feat_1_32 = self.down32(x)  # 16×16

        return [feat_1_4, feat_1_8, feat_1_16, feat_1_32]


    def forward(self, x):
        self.caculate_mask_all()

        fea_list = self.forward_features(x)
    ...
    def configure_latency(self, latency, tau):
        ...
        self.hard = True
        for layer_idx in range(self.depth):
            self.blocks[layer_idx].attn.configure_latency(latency, tau=tau, hard=True)
            self.blocks[layer_idx].mlp.configure_latency(latency, tau=tau, hard=True)
...
if __name__ == '__main__':
    ...
    embed_mask = torch.tensor([1.0]*10+[0.0]*2).to(device)

    mask_attn = torch.tensor([1.0]*10+[0.0]*2).to(device)
    mask_mlp = torch.tensor([1.0]*6+[0.0]*2).to(device)
