...
sys.path.append("/mnt/nas_jiasheng/zhaowangbo/zhuchen/video_v1")
...
from utils.eval_flag import eval_mat, eval_mat_fined, eval_mat_combined, eval_mat_combined_dis, eval_macs_loss_dis
import random
...
def train(args):
    ...
    trainDataLoader = DataLoader(dataset_train, args.batch_size, sampler=DistributedSampler(dataset_train))
    valDataLoader = DataLoader(dataset_val, args.batch_size, sampler=DistributedSampler(dataset_val))
    ...
    for epoch in range(args.epochs):
        ...
        if epoch % 2 == 0:
            for index, f in enumerate(flags_list):
                ...
                eval_mat_combined_dis(model, valDataLoader, criterion, epoch, optimizer, args, flag=f, device=device, local_rank=local_rank, sub_dim=sub_dim, depth_list=depth_list, mlp_ratio=mlp_ratio,
                                           mha_head=mha_head)
